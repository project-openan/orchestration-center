...
async def register(request: RegisterRequest):
    # Self-registration is disabled by default: an open register endpoint on
    # the auth whitelist would let anyone mint a valid session and negate the
    # whole internal auth gate. Operators opt in via server.conf.
    conf = get_conf()
...
async def auth_check(request: Request):
    conf = get_conf()
    registration_enabled = _registration_enabled(conf)
    if not is_auth_enabled():
...
def _verify_access_password(stored: str, plaintext: str) -> bool:
    """Verify the file-mode access password.

    Supports versioned bcrypt hashes, older direct-bcrypt hashes, and the
    legacy sha256(plaintext) values generated by generate_access_password.py.
    """
    if not stored:
        return False
    if stored.startswith(FILE_HASH_PREFIX):
        return verify_password(plaintext, stored[len(FILE_HASH_PREFIX):])
    if stored.startswith("$2"):
        return verify_legacy_bcrypt(plaintext, stored)
    return secrets.compare_digest(hashlib.sha256(plaintext.encode()).hexdigest(), stored)


def _hash_access_password(plaintext: str) -> str:
    """Hash a file-mode access password with an explicit scheme marker."""
    return FILE_HASH_PREFIX + hash_password(plaintext)
...
async def change_password(request: ChangePasswordRequest, http_request: Request):
    ...
    # File mode: update access_password in server.conf. Verify the incoming
    # old password against the stored hash (bcrypt current, legacy sha256
    # supported), then store the new password as a bcrypt hash.
    stored = conf.get("access_password", "")
    if not _verify_access_password(stored, request.old_password):
        raise HTTPException(status_code=401, detail="Current password is incorrect")
    new_hash = _hash_access_password(request.new_password)
    ...
    if _update_access_password_in_conf(conf_path, new_hash):
...
async def generate_from_preflow(
    ...
    try:
        ...
        # 串行多次同步 LLM 调用(分钟级),卸载到线程避免阻塞事件循环
        workflow = await anyio.to_thread.run_sync(
            generator.generate_psop_workflow,
...
async def generate_from_intent(
    ...
    try:
        ...
        psop = await anyio.to_thread.run_sync(
            partial(
                generator.generate_psop_from_intent,
...
async def retrieve_by_intent(
    ...
    try:
        ...
        psop = await anyio.to_thread.run_sync(
            SharedHandlers.retrieval().retrieve_psop_by_intent, request.user_intent,
            abandon_on_cancel=False,
        )
        if not psop:
...
async def retrieve_topn_by_intent(
    ...
    try:
        ...
        results: List[WorkflowSearchResult] = await anyio.to_thread.run_sync(
            SharedHandlers.retrieval().retrieve_psop_by_intent_topn, request.user_intent, top_n,
            abandon_on_cancel=False,
        )
